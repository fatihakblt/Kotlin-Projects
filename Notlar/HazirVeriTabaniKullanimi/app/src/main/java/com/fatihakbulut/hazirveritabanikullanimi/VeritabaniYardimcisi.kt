...
class VeritabaniYardimcisi(context:Context) : SQLiteOpenHelper(context,"filmler.sqlite",null,1) {
    ...
    // Tablolara bir sorun olursa ne yapmamız gerektiğini söylediğimiz yer.
    override fun onUpgrade(db: SQLiteDatabase?, oldVersion: Int, newVersion: Int) {

        db?.execSQL("DROP TABLE IF EXISTS filmler") // Sistemde bir sorun olursa tablo varsa sil
        db?.execSQL("DROP TABLE IF EXISTS kategoriler")
        db?.execSQL("DROP TABLE IF EXISTS yonetmenler")

        onCreate(db) // Yukarıdaki veritabanı tabloları yeniden oluşturulacak
    }
